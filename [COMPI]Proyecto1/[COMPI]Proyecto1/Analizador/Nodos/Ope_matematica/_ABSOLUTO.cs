...
namespace _COMPI_Proyecto1.Analizador.Nodos.Ope_matematica
{
    ...
    */
    {
        public _ABSOLUTO(string nombre, tablaSimbolos tabla) : base(nombre, tabla)
        {
        }
        ...
        */
        ...
        public override itemValor ope_tipo(elementoEntorno elem)
        {
            ...
            {
                retorno.setValue(Math.Abs(itemInicio.getDecimal()));
                return retorno;
            }
            else if (inicioTemp != null)
            {
                inicioVal = (int)inicioTemp;
                retorno.setValue(Math.Abs(inicioVal));
                return retorno;
            }
            else
            {
