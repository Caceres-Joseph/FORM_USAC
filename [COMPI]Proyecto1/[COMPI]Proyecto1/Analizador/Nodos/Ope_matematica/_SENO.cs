...
namespace _COMPI_Proyecto1.Analizador.Nodos.Ope_matematica
{
    ...
    */
    {
        public _SENO(string nombre, tablaSimbolos tabla) : base(nombre, tabla)
        {
        }
        ...
        */
        ...
        public override itemValor ope_tipo(elementoEntorno elem)
        {
            ...
            {
                retorno.setValue(Math.Sin(itemInicio.getDecimal()));
                return retorno;
            }
            else if (inicioTemp != null)
            {
                inicioVal = (int)inicioTemp;
                retorno.setValue(Math.Sin(inicioVal));
                return retorno;
            }
            else
            {
